...
class APPSingleton {
    ...
    func createUM() {
        ...
        UMConfigure.initWithAppkey("60bae3094d0228352bbcc977", channel: "App Store")
...
//        UMSocialGlobal.shareInstance()?.universalLinkDic = [UMSocialPlatformType.wechatSession.rawValue:"https://stu-link.jledu.com/qmschool/weichat/"]
        ...
        // wx49931533a8d3600b
        // 66396f25d956a31da5d1ddb959165da5
        
//        UMSocialManager.default()?.setPlaform(.wechatSession,
//                                                           appKey: "wx49931533a8d3600b",
//                                                           appSecret: "66396f25d956a31da5d1ddb959165da5",
//                                                           redirectURL: nil)
    ...
    var role:Role {
        get{
            if let value = UserDefaults.standard.value(forKey: KEY_ROLE){
                
                return Role(rawValue: (value is String ? (value as! String) : "t")) ?? .teacher
            }
            else{
                return Role(rawValue: "t") ?? .teacher
            }
        }
        set{
            UserDefaults.standard.setValue(newValue.rawValue, forKey: KEY_ROLE)
            UserDefaults.standard.synchronize()
        }
    }
    ...
    func requestRegistUser(userid:Int,
        ...
        manager.POSTRequest(URLString: INTERFACE_registUser,
            ...
            AppPrint(JSONSerialization.JSONString(object: requestData))
            let code: Int? = requestData["code"] as? Int
            let message:String = requestData["msg"] as? String ?? "code:\(String(describing: code))"
            if StatusCode.succeed(code: code){
                if let token:String = requestData["data"] as? String {
                    APPSingleton.shared.QMToken = token
                    completion(true, nil, message, nil)
                }
                else {
